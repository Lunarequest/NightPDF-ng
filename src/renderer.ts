...
const nightPDF = (function () {
    ...
    function main() {
        ...
        window.api.on('file-open', (_e: string, msg: string) => {
            _openFile(msg);
        });

        window.api.removeAllListeners('file-print');
        window.api.on('file-print', (_e: string, _msg: string) => {
            _pdfElement.ownerDocument
    ...
    const _updateDarkSettings = (cssFilter: string) => {
        const currentStyle =
            _pdfElement.ownerDocument.getElementById('pageStyle');
